...
// getPostByID and return comments with user and post with user
...
exports.deleteLike = async (req,res,next) => {
    try {
        ...
        const likeIndex = post.likes.findIndex(like => like.id === req.params.id);

        if (likeIndex === -1) {
            resourceNotFound(`No Like Found with ID = ${req.params.id}`);
          }
      
          // CHECK IF UNAUTHORIZED USER
          if (post.likes[likeIndex].user.toString() !== req.id) {
            const error = new Error("Not Authorized User");
            error.statusCode = 403;
            throw error;
          }
      
          post.likes.splice(likeIndex, 1); // Remove the like at the specified index
          let likedPost = await post.save();
          likedPost = await Post.findById(likedPost._id).populate('user comments.user', '-password');
